...
public final class FastFibonacci {
	 ...
	 * (2*F(n+1) - F(n)) F(2n+1) = F(n+1)^2 + F(n)^2
	 */
	...
	/*
	 * Fast matrix method. Easy to describe, but has a constant factor slowdown
	 * compared to doubling method. [1 1]^n [F(n+1) F(n) ] [1 0] = [F(n) F(n-1)]
	 */
	...
	private static BigInteger[] multiply(BigInteger[] x, BigInteger[] y) {
		return new BigInteger[] { multiply(x[0], y[0]).add(multiply(x[1], y[2])),
				multiply(x[0], y[1]).add(multiply(x[1], y[3])), multiply(x[2], y[0]).add(multiply(x[3], y[2])),
				multiply(x[2], y[1]).add(multiply(x[3], y[3])) };
	}

	/*
	 * Simple slow method, using dynamic programming F(n+2) = F(n+1) + F(n)
	 */
	...
	private static BigInteger multiply(BigInteger x, BigInteger y) {
		return x.multiply(y); // Replace this line with Karatsuba
								// multiplication, etc. if available
	}
